# Tosumu Software Design Document

| Field | Value |
| --- | --- |
| Authority | Normative architecture and design source of truth |
| Lifecycle | Current, pre-stability; includes labeled deferred and speculative sections |
...
See the [documentation status dashboard](../document-status.md) for the
current decisions, reviews, plans, and the incremental design-decomposition
work. Future material in this document is not an implementation guarantee.
...
>
> **Published at** https://github.com/Arakendo/tosumu. Dual-licensed MIT OR Apache-2.0. This is a public, pre-audit database project: the crypto and storage design are documented, but neither has been independently reviewed or audited. Do not use `tosumu` to protect real secrets; see the repository-root `SECURITY.md` and the published [Safety and Limits](../safety-and-limits.md) summary.
...
The operator-facing sibling surface is TQL, documented separately in the [Tosumu Command Language](../Tosumu%20Command%20Language.md) proposal. The intended split is: SQL retrieves; TQL explains and operates.
...
The audit also emits a numeric **entropy report** alongside the findings — see §12.5 for the full set of metrics. On the inspect contract this is additive: `inspect.audit` gains a top-level `entropy: { ... }` payload object, leaving every existing field untouched (per the [Tosumu Inspect API Specification](Tosumu%20Inspect%20API%20Specification.md) compatibility rules).
...
- **Reference:** See [Tosumu Reference Implementations](Tosumu%20Reference%20Implementations.md) for LruCache (page cache eviction pattern) and RingBuffer (optional WAL buffering).
...
- **Reference:** See [Tosumu Reference Implementations](Tosumu%20Reference%20Implementations.md) for BPlusTree (node splitting, leaf links, rebalancing algorithms).
...
The detailed design lives in the [Tosumu Command Language](../Tosumu%20Command%20Language.md) proposal.
...
The current ordinary-open interval is exactly format 3. Format 2 is refused
before recovery or mutation; Tosumu performs no automatic or in-place upgrade.
...
├── docs/
│   └── Specifications/
│       └── Tosumu Software Design Document.md  (this file)
├── README.md
├── SECURITY.md
├── LICENSE-MIT
├── LICENSE-APACHE
...
**Source of truth vs. derived artifacts.** The Tosumu Software Design Document is the hand-authored source of truth. The codebase is derived from it. Before Stage 2 is complete and the on-disk format is locked, this document wins when it conflicts with the implementation — the implementation follows the design, not the other way around. Discrepancies are bugs to fix, not justification for retroactive spec updates.

After Stage 2, the relationship partially inverts in one direction: the implemented and tested on-disk format is the ground truth for that format version, and this document must be kept in sync with it. Format documentation that diverges from the actual bytes on disk is worse than no documentation.
