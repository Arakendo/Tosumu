# Tosumu Reference Implementations

| Field | Value |
| --- | --- |
| Authority | Informative |
| Lifecycle | Current |
| Scope | External implementations and resources that inform Tosumu work |
...
**Relevance:** The Tosumu Software Design Document §17.3 mentions spatial indexes as a "reasonable Stage 7+ extension." This is a reference implementation for bounding-box splits and nearest-neighbor queries.
...
**Relevance:** Simpler than FSTs (Tosumu Software Design Document §17.2.1), incrementally mutable. Could be adapted to page-based storage for autocomplete-style queries on string keys. Less compact than FSTs but easier to implement.
...
- **Graph structures** — tosumu is key/value, not graph traversal (Tosumu Software Design Document §17.4 explicitly rules this out).
...
- Tosumu is explicitly single-process (Tosumu Software Design Document §1.2).
...
| VaultServices | Tosumu Software Design Document §8 |
|---|---|
...
**This library implements the same envelope encryption architecture described by the Tosumu Software Design Document §8.** The parallels are striking:
...
**Verdict:** Out of scope per §17.2.2. Already mentioned in the Tosumu Software Design Document.
...
A constructed language project. Not a code library — a design artifact. Referenced here because the design decisions made during Tonesu's development produced transferable lessons that have been directly incorporated into the Tosumu Software Design Document.
...
### Lessons incorporated into the Tosumu Software Design Document

These are not analogies — they are direct derivations. The Tosumu Software Design Document sections listed are where each lesson was applied.
...
|------|-------------------|
| `spec/principles.md` | The 9 normative principles of the language — compare directly with Tosumu Software Design Document §2 |
...
1. **Before starting a stage**, read the Tosumu Software Design Document section for that stage.
