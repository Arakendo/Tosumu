...
fn prompt_unlock_secret(kind: UnlockKind) -> Result<UnlockSecret, TosumuError> {
    match kind {
        ...
        UnlockKind::Keyfile => Ok(UnlockSecret::Keyfile(prompt_keyfile_path("keyfile path: ")?)),
    }
}
...
fn open_page_store_readonly(path: &Path, unlock: Option<&UnlockSecret>) -> Result<PageStore, TosumuError> {
...
fn open_page_store_writable(path: &Path, unlock: Option<&UnlockSecret>) -> Result<PageStore, TosumuError> {
    open_with_unlock_secret(
        unlock,
        || PageStore::open(path),
...
pub(crate) fn open_btree_with_unlock(path: &Path, unlock: Option<&UnlockSecret>) -> Result<BTree, TosumuError> {
    open_with_unlock_secret(
        unlock,
    ...
    fn unlock_fallback_returns_first_successful_prompted_secret() {
        ...
        let (value, unlock) = open_with_unlock_fallback(
            ...
            |kind| {
                prompted.push(kind);
                Ok(match kind {
                    UnlockKind::Passphrase => UnlockSecret::Passphrase("passphrase-secret".to_string()),
                    UnlockKind::RecoveryKey => UnlockSecret::RecoveryKey("recovery-secret".to_string()),
        ...
        assert_eq!(unlock, Some(UnlockSecret::RecoveryKey("recovery-secret".to_string())));
        assert_eq!(prompted, vec![UnlockKind::Passphrase, UnlockKind::RecoveryKey]);
    }
