...
pub(crate) fn run_init(path: &Path, encrypt: bool) -> Result<(), TosumuError> {
    if encrypt {
...
pub(crate) fn run_put(path: &Path, key: &str, value: &str) -> Result<(), TosumuError> {
    let mut store = open_store_writable(path)?;
    store.put(key.as_bytes(), value.as_bytes())?;
    Ok(())
}

pub(crate) fn run_get(path: &Path, key: &str) -> Result<(), TosumuError> {
...
pub(crate) fn run_delete(path: &Path, key: &str) -> Result<(), TosumuError> {
    let mut store = open_store_writable(path)?;
    store.delete(key.as_bytes())?;
    Ok(())
}

pub(crate) fn run_scan(path: &Path) -> Result<(), TosumuError> {
    let store = open_store_readonly(path)?;
...
pub(crate) fn run_stat(path: &Path) -> Result<(), TosumuError> {
    let store = open_store_readonly(path)?;
